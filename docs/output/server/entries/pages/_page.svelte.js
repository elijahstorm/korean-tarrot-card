import { c as create_ssr_component, d as add_attribute, a as validate_store, b as subscribe, f as each, v as validate_component, e as escape, m as missing_component } from "../../chunks/index2.js";
...
const Click_indicators = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  validate_store(showCardDisplay, "showCardDisplay");
  ...
  validate_store(roadmapData, "roadmapData");
  ...
  return `${$showCardDisplay ? `${each($indicators, (indicator) => {
...
const Nav = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_showNav();
  return `${$showNav ? `<nav class="flex gap-6 text-white"><button type="button"><img src="${escape(base, true) + "/nav/back-button.svg"}" alt="Back button"></button>
		<button type="button"><img src="${escape(base, true) + "/nav/home-button.svg"}" alt="Home button"></button>
		<button type="button"><img src="${escape(base, true) + "/nav/info-button.svg"}" alt="Info button"></button></nav>` : ``}`;
});
const Lang = {
  title: "동양의 타로타드",
...
const Roadmap = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  validate_store(roadmapData, "roadmapData");
  $$unsubscribe_roadmapData = subscribe(roadmapData, (value) => $roadmapData = value);
  validate_store(selectedCardsState, "selectedCardsState");
  $$unsubscribe_selectedCardsState = subscribe(selectedCardsState, (value) => $selectedCardsState = value);
  validate_store(currentDisplayedScene, "currentDisplayedScene");
...
const Bg_video = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="absolute inset-0 w-full h-full overflow-hidden"><video autoplay muted${add_attribute("src", base + "/" + src, 0)} class="${[
...
const End_scene = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<div class="w-full h-full bg-black"></div>`;
});
const Prediction_scene = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_currentDisplayedScene();
  $$unsubscribe_roadmapData();
  return `<div class="bg-black absolute inset-0 w-full h-full overflow-hidden"><img class="absolute top-1/2 left-1/2 min-w-full min-h-full w-auto h-auto -translate-x-1/2 -translate-y-1/2" src="${escape(base, true) + "/backgrounds/cards/cards-bg.png"}" alt="card choice background"></div>
...
const css$2 = {
  code: ".intro-header.s-PHsQqkHR-Q5f{-webkit-mask-image:-webkit-gradient(\n			linear,\n			center top,\n			center bottom,\n			from(rgba(0, 0, 0, 1)),\n			to(rgba(0, 0, 0, 0))\n		)}",
  map: null
};
...
const Prediction_content = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  validate_store(roadmapData, "roadmapData");
  $$unsubscribe_roadmapData = subscribe(roadmapData, (value) => $roadmapData = value);
  validate_store(seenCardsState, "seenCardsState");
  $$unsubscribe_seenCardsState = subscribe(seenCardsState, (value) => $seenCardsState = value);
  $$unsubscribe_showCardDisplay();
  $$unsubscribe_roadmapData();
  $$unsubscribe_seenCardsState();
  return `${$showCardDisplay ? `<div class="flex h-full justify-end items-end relative bottom-0">${[...new Set($roadmapData?.seenItems)].length >= 3 ? `<div class="flex gap-8 pr-8"><button class="rounded-full border border-zinc-500 hover:bg-zinc-700 overflow-clip h-12 w-12" type="button"><img src="${escape(base, true) + "/nav/repeat.png"}" alt="repeat" style="object-position: -0.1px 0"></button>
...
const css$1 = {
  code: ".responsive.s-Ud7VFCebVPWE{height:20vh;max-height:120px}",
  map: null
};
...
<div class="flex flex-col"><div class="flex-1 flex flex-col py-8"><div class="responsive col-span-3 grid justify-center s-Ud7VFCebVPWE">${cards.length === 0 ? `<div class="col-start-1 row-start-1 invisible"></div>` : `${each(cards, (card, index) => {
    return `<div class="card responsive col-start-1 row-start-1 s-Ud7VFCebVPWE">${slots.length < 3 ? `<button type="button" class="${[
      "card responsive border border-black transition-all s-Ud7VFCebVPWE",
      card.selected ? "hidden" : ""
    ].join(" ").trim()}"${add_attribute("style", cardRotation(card), 0)}><img src="${escape(base, true) + "/cards/behind.png"}" alt="card background">
							</button>` : ``}
					</div>`;
  })}`}</div>

		<div class="responsive self-center col-span-3 grid grid-cols-1 mt-14 pt-16 transition-all duration-700 s-Ud7VFCebVPWE"${add_attribute("style", "", 0)}>${slots.length === 0 ? `<div class="card responsive invisible s-Ud7VFCebVPWE"></div>` : `${each(slots, (card, index) => {
    return `<div class="card responsive col-start-1 row-start-1 grid border border-black transition-all s-Ud7VFCebVPWE" style="${"transition-duration: " + escape(duration, true) + "ms; " + escape(
      ...
      " scale-x-100"
    ].join(" ").trim()}" src="${escape(base, true) + "/cards/behind.png"}" alt="card background" style="${"transition-duration: " + escape(duration / 4, true) + "ms; transition-delay: " + escape(index * duration / 4, true) + "ms"}">

						<img class="${[
      "col-start-1 row-start-1 transition-all dur",
      " scale-x-0"
    ].join(" ").trim()}"${add_attribute("src", base + `/cards/${"small/"}${$selectedCardsState[index].fileName}.png`, 0)}${add_attribute("alt", $selectedCardsState[index].title, 0)} style="${"transition-duration: " + escape(duration / 4, true) + "ms; transition-delay: " + escape((1 + index) * duration / 4, true) + "ms"}">
					</div>`;
  })}`}</div>
    ...
    return `<div class="flex flex-col col-start-1 row-start-1 space-y-2 items-center" style="${"translate: calc(" + escape(index, true) + " * (min(20vh, 120px) * (0.617 + 0.25)) - min(20vh, 120px)) 0"}"><p class="font-maruburi">${escape(name)}</p>
						...
						${!slots[index] ? `<div class="card responsive border-2 border-dashed border-zinc-600 s-Ud7VFCebVPWE"></div>` : ``}
					</div>`;
  })}</div>`}</div>
</div>`;
});
const Results_content = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  validate_store(selectedCardsState, "selectedCardsState");
  $$unsubscribe_selectedCardsState = subscribe(selectedCardsState, (value) => $selectedCardsState = value);
  validate_store(predictionState, "predictionState");
  ...
  return `<div class="flex flex-col gap-8 h-full"><section class="flex-1 flex gap-8"><div class="flex gap-4" style="flex: 3">${each($selectedCardsState, (card, index) => {
    return `<div class="flex-1 flex flex-col gap-4 justify-center"><div><h5 class="text-center font-maruburi">${escape(Lang.timelineNames[index])}</h5>
					...
					<img${add_attribute("src", base + `/cards/small/${card.fileName}.png`, 0)}${add_attribute("alt", card.title, 0)}>
				</div>`;
  })}</div>
    ...
    return `<div class="flex flex-col gap-2"><div class="bg-zinc-700 rounded-md border border-zinc-600 p-2"><img${add_attribute("src", base + `/items/${item(card).fileName}.png`, 0)}${add_attribute("alt", item(card).name, 0)} class="aspect-1 w-16"></div>
...
const css = {
  code: ".octagon.s-hneWGYE28Tsp{clip-path:polygon(30% 0, 70% 0, 100% 30%, 100% 70%, 70% 100%, 30% 100%, 0 70%, 0 30%);display:flex;justify-content:center;align-items:center;border:none}",
  map: null
};
const Customize_content = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<div class="flex gap-4 pt-8"><div class="flex-1"><div class="${"card rounded-[3.5rem] max-h-full p-4 m-4 ring-offset-2 ring-gray-600 ring-offset-black ring-2 ring-opacity-100 " + escape(selectedAttribute, true) + " s-hneWGYE28Tsp"}"></div></div>

	<div class="flex flex-col gap-12 text-xs"><div class="flex gap-4 custom-scrollbar p-1 pb-4">${each(attributes, (attribute, index) => {
    return `<button class="${[
      "btn w-14 h-36 rounded overflow-clip ring-2 ring-offset-2 ring-offset-black " + escape(attribute.value, true) + " s-hneWGYE28Tsp",
    ...
    return `<div class="${[
      "octagon aspect-1 w-[6.6rem] transition s-hneWGYE28Tsp",
      (selectedAttribute !== attribute.value ? "bg-gray-600" : "") + " " + (selectedAttribute === attribute.value ? "bg-white" : "")
    ].join(" ").trim()}"><div class="octagon aspect-1 w-[6.4rem] bg-black s-hneWGYE28Tsp"><button class="${"btn aspect-1 w-24 octagon px-4 py-2 hover:bg-gray-200 mt-[0.2rem] overflow-clip ring-1 ring-offset-1 ring-gray-400 ring-offset-gray-300 " + escape(attribute.value, true) + " s-hneWGYE28Tsp"}" type="button">${escape(attribute.name)}
						</button></div>
				</div>`;
  })}</div></div>
</div>`;
...
const Selection_scene = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  validate_store(videoState, "videoState");
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  validate_store(viewState, "viewState");
